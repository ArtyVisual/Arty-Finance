...
const Addinc = () => {
  ...
  useEffect(() => {
    ...
    setIncomeArray(storedIncomes);
  }, []);

  const saveIncome = () => {
    if (form.amount > 0 && !isNaN(form.amount) && form.desc.length > 3 && form.mode.length > 3 && form.date)  {

    const newIncome = { ...form, id: uuidv4() };
    const updatedIncome = [...IncomeArray, newIncome];

    setIncomeArray(updatedIncome);
    localStorage.setItem("incomes", JSON.stringify(updatedIncome));
  ...
  <>
    ...
    />
    <div>
      <h1 className='text-center md:text-5xl text-2xl font-bold text-zinc-300 my-12'>Enter Your Income Details Here </h1>
      ...
      <div className="grid my-12 w-full justify-center py-2 gap-4 mb-4">
          <input
            ...
            name="amount"
            type="number"
            className="text rounded-lg border-2 md:w-60 px-auto m-4 text-center py-2 font-semibold text-slate-900 border-white p-2 bg-green-200"
        ...
        <input
          ...
          name="date"
          type="date"
          className="last:text rounded-lg border-2 md:w-60 px-auto m-4 text-center py-2 font-semibold  text-slate-900 border-white p-2 bg-green-200"
